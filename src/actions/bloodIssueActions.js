...
export const listBloodIssue = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`https://blooddonationapp123.herokuapp.com/api/blood-issue`)
...
export const createBloodIssue = (blood) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`https://blooddonationapp123.herokuapp.com/api/blood-issue`, blood, config)
...
export const deleteBloodIssue = (id) => async (dispatch, getState) => {
  try {
    ...
    await axios.delete(`https://blooddonationapp123.herokuapp.com/api/blood-issue/${id}`, config)
